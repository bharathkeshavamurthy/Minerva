...
\nm{i.i.d. across frequency? Or is there any freq. correlation?}
...
\nm{Please use $\mathbb P$ for probabilities instead of $P$}
...
\nm{I will rather write it as:}
\add{
\begin{align}
\mathbb P(\vec{X}=\vec{x})=\mathbb P(X_1=x_1)\prod_{j=2}^K \mathbb P(X_{j}=x_j|X_{j-1}=x_{j-1})
\end{align}
so that $\vec{X}$ obeys the Markov property across frequency.
We also let $\mathbb P(X_{j}=1|X_{j-1}=0)=p$ and $\mathbb P(X_{j}=0|X_{j-1}=1)=q$,
and $\mathbb P(X_1=1)=\frac{p}{p+q}$. Note that this decomposition implies that 
\begin{align}
\mathbb P(X_k=1)=\frac{p}{p+q},\ \forall k
\end{align}
and also
\begin{align}
\mathbb P(\vec{X}=\vec{x})=\mathbb P(X_K=x_K)\prod_{j=1}^{K-1} \mathbb P(X_{j}=x_j|X_{j+1}=x_{j+1})
\end{align}
so that it obeys the Markov property also in the reversed direction.
}
...
\nm{please use $\mathbb E$ for expectation}
\[E[|X_k(i)|^2]\ =\ x_k,\ given\ X_k\ during\ observation\ cycle\ i\ has\ realized\ as\ x_k\]
\nm{This should be written as $\mathbb E[|X_k(i)|^2|X_k=x_k]\ =\ x_k$ (i.e., make the condition explicit in the expectation so there is no need to explain); however this result is obvious and you should remove it}
...
\nm{errors below, please take a careful look at my corrections:}
\begin{align*}
    Var[Y_k(i) | X_k(i) = x_k] = E[(Y_k(i) | X_k(i) = x_k)^2] - [E[Y_k(i) | X_k(i) = x_k]]^2
    \\
\add{Var[Y_k(i) | X_k(i) = x_k] = \mathbb E[|Y_k(i)|^2 | X_k(i) = x_k] - |\mathbb E[Y_k(i) | X_k(i) = x_k]|^2}    
\end{align*}
\begin{align*}
    Var[Y_k(i) | X_k(i) = x_k] = E[|H_kX_k(i)|^2 + |V_k(i)|^2 + 2H_kX_k(i)V_k(i)] - (0)^2
    \\
    \add{Var[Y_k(i) | X_k(i) = x_k] = \mathbb E[|H_kX_k(i)|^2 + |V_k(i)|^2 + 2\text{real}(H_kX_k(i)V_k^*(i))] - (0)^2}
\end{align*}
\nm{remember you are dealing with complex variables}
\begin{align*}
    Var[Y_k(i) | X_k(i) = x_k] = \sigma_H^2E[|X_k(i)|^2] + \sigma_V^2 + 2E[H_k]E[X_k(i)]E[V_k(i)]
    \\
    \add{    Var[Y_k(i) | X_k(i) = x_k] = \sigma_H^2\mathbb E[|X_k(i)|^2|X_k=x_k] + \sigma_V^2 + 2\text{real}(\mathbb E[H_k]E[X_k(i)]E[V_k(i)^*])}
\end{align*}
...
The following visualization results illustrate the Occupancy Behavior of the Primary User in a wideband spectrum of interest consisting of 18 frequency bands observed over 100 sampling rounds.
\nm{since it is static, no need to show evolution over time...}
 The PU behavior is assumed to be static (constant across time).
\newline
\begin{itemize}
    ...
    \nm{what about $\mathbb P(Idle|Occupied)$?}
...
\nm{remember you are dealing with complex vars!}
\nm{Before explaining which algorithm you are going to use, introduce the problem you are going to solve:}
\add{Given a sequence of observations $\vec{Y}=\vec{y}$ across frequency, we want to develop a maximum a posteriori estimator of the frequency occupancy vector $\vec{X}$, i.e.
\begin{align}
\vec{X}^*=\arg\max_{\vec{x}}\mathbb P(\vec{X}=\vec{x}|\vec{Y}=\vec{y})
\end{align}
...
\begin{equation}\label{57}
    m_{X_k(i)=x_k=l}(Y_k(i)=y_k)\ \sim \mathcal{N}(0,\ \sigma_H^2l + \sigma_V^2)
\end{equation}
...
\begin{equation}\label{59}
    \Pi\ =\ P(X_1\ =\ 1)\ is\ the\ initial\ PU\ occupancy\ probability
\end{equation}
\subsection{The Estimator}
...
\begin{equation}\label{60}
    F(j,\ l)\ \triangleq \ P(y_1,\ y_2,\ y_3,\ ....,\ y_i,\ y_j,\ x_j=l)
\end{equation}
Using the definition of Marginal Probability, equation (\ref{60}) can be written as,
\begin{equation}\label{61}
    F(j,\ l)\ = \ \sum_{r\in\{0,1\}}\ P(y_1,\ y_2,\ y_3,\ ....,\ y_i,\ y_j,\ x_j=l,\ x_i=r)
\end{equation}
Using the definition of conditional probability, equation (\ref{61}) can be written as,
\begin{equation}\label{62}
    F(j,\ l)\ = \ \sum_{r\in\{0,1\}}\ P(x_j=l,\ y_j\ |\ y_1,\ y_2,\ y_3,\ .....,\ y_i,\ x_i=r)P(y_1,\ y_2,\ y_3,\ .....,\ y_i,\ x_i=r)
\end{equation}
...
\begin{equation}\label{63}
    F(j,\ l)\ = \ \sum_{r\in\{0,1\}}\ P(x_j=l,\ y_j\ |\ x_i=r)F(i,\ r)
\end{equation}
...
\begin{equation}\label{64}
    B(j,\ r)\ \triangleq\ P(y_j,\ y_{j+1},\ y_{j+2},\ ......,\ y_K\ |\ x_i\ =\ r)
\end{equation}
Using the definition of Marginal Probabilities,
\begin{equation}\label{65}
    B(j,\ r)\ =\ \sum_{l\in\{0,1\}} P(y_j,\ y_{j+1},\ y_{j+2},\ ......,\ y_K,\ x_j=l\ |\ x_i\ =\ r)
\end{equation}
Now, re-arranging the terms in equation (\ref{65}), we get,
\begin{equation}\label{66}
    B(j,\ r)\ =\ \sum_{l\in\{0,1\}} P(y_{j+1},\ y_{j+2},\ ......,\ y_K,\ y_j,\ x_j=l\ |\ x_i\ =\ r)
\end{equation}
Now, we know that,
\begin{equation*}
...
\begin{equation}\label{70}
    P(x_j=l,\ y_j\ |\ x_i=r)\ =\ P(y_j\ |\ x_j=l,\ x_i=r)P(x_j=l\ |\ x_i=r)
\end{equation}
...
\begin{equation}\label{71}
    P(x_j=l,\ y_j\ |\ x_i=r)\ =\ m_{l}(y_j)a_{rl}
\end{equation}
We know the emission probabilities from equation (\ref{57}) as,
\[m_l(y_j)\ \sim\ \mathcal{N}(0,\ \sigma_H^2l + \sigma_V^2)\]
Now, we can find a reasonably good estimate of $P(x_j=l,\ y_j\ |\ x_i=r)$ using the following expression.
\begin{equation*}
    P(x_j=l,\ y_j\ |\ x_i=r)\ =\ \frac{number\ of\ transitions\ from\ x_i=r\ to\ x_j=l\ emitting\ symbol\ y_j}{total\ number\ of\ transitions\ from\ x_i=r}
\end{equation*}
Mathematically, the above equation can be written as,
\begin{equation}\label{72}
    P(x_j=l,\ y_j\ |\ x_i=r)\ =\ \frac{n(x_i=r,\ x_j=l,\ y_j)}{\sum_{s\in\{0,1\}}\ \sum_{\Tilde{y}\in\vec{y}} n(x_i=r,\Tilde{y},x_j=s)}
\end{equation}
Assuming we have multiple estimates of the state sequence, we can weight the count using the probability of that state sequence, i.e. $P(path)\ =\ P(\vec{X}=\vec{x}\ |\ \vec{Y}=\vec{y})$,
\begin{equation}\label{73}
    P(x_j=l,\ y_j\ |\ x_i=r)\ =\ \frac{\sum_{\vec{x}}\ P(\vec{x}\ |\ \vec{y})\ n(x_i=r,\ x_j=l,\ y_j)}{\sum_{s\in\{0,1\}}\ \sum_{\Tilde{y}\in\vec{y}}\ \sum_{\vec{x}}\ P(\vec{x}\ |\ \vec{y})\ n(x_i=r,\Tilde{y},x_j=s)}
\end{equation}
