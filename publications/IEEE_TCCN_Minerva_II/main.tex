...
\begin{algorithm}[t]
    ...
    \label{Alg. PERSEUS}
    \begin{flushleft}
        \textbf{Fragmented POMDP Model: } $(\mathcal{B}_{\Delta},\mathcal{A}_{\Delta},\mathcal{Y}_{\Delta},\mathbf{A}_{\Delta},\mathbf{M})$ \Comment{$\mathbf{A}_{\Delta}$ parameterized by $\hat{\vec{\theta}}$}\\
    ...
    \end{flushleft}
    \begin{algorithmic}[1]
        \State{Determine the set of \emph{reachable beliefs} $\tilde{\mathcal{B}}$.} \Comment{Random exploration}
        \State{$V_{0}(\beta){=}\frac{-K\lambda}{1{-}\gamma},{\forall}\beta{\in}\tilde{\mathcal{B}}.$} \Comment{Initialization}
        \While{$|V_{t{+}1}(\beta){-}V_{t}(\beta)|{<}\epsilon,{\forall}\beta{\in}\tilde{\mathcal{B}},\ \epsilon{>}0,\ \text{Iteration index}{=}t$}
            \State{$\tilde{\mathcal{U}}{\longleftarrow}\tilde{\mathcal{B}}$} \Comment{Set copy for local updates}
            \While{$\tilde{\mathcal{U}}{\neq}\{\}$} \Comment{Improve all points in $\tilde{\mathcal{U}}$}
                \State{$\beta_{u}{=}\text{random.choice}(\tilde{\mathcal{U}});\ \vec{\alpha}_{t+1}^{u}{=}\xi_{\mathcal K_{t+1}^{u}}^{u};\ \mathcal{K}_{t+1}^{u}{=}\argmax_{\mathcal{K}{\in}\mathcal{A}} \beta_{u}{\cdot}\xi_{\mathcal{K}}^{u}$, where} \Comment{Backup}
                \State{$\xi_{\mathcal{K}}^{u}(\vec{B}){=}\mathbb{E}_{\vec{Y}|\vec{B},\mathcal{K}}\Big[R(\hat{\mathbb{B}}(\beta_{u},\mathcal{K},\vec{Y})){+}\gamma\sum_{\vec{B}'}\mathbb{P}(\vec{B}(i{+}1){=}\vec{B}'|\vec{B}(i){=}\vec{B})\xi_{\mathcal{K},\vec{Y}}^{u}(\vec{B}')\Big]$, and}
                \State{$\xi_{\mathcal{K},\vec{Y}}^{u}{=}\argmax_{\alpha_{t}^{u'},u'{\in}\{1,2,\dots,|\tilde{\mathcal{B}}|\}}\mathbb{B}(\hat{\mathbb{B}}(\beta_{u},\mathcal{K},\vec{Y})){\cdot}\alpha_{t}^{u'}$.} \Comment{Future hyperplane}
                \For{$\beta'{\in}\tilde{\mathcal{U}}$}
                    \State{$V_{t+1}(\beta'){:=}V_{t}(\beta')$; $\vec{\alpha}_{t+1}(\beta'){:=}\vec{\alpha}_{t}(\beta')$; $\mathcal{K}_{t+1}(\beta'){:=}\mathcal{K}_{t}(\beta').$} \Comment{Persist}
                    \If{$\beta'{\cdot}\vec{\alpha}_{t+1}^{u}{\geq}V_{t}(\beta')$} \Comment{Improvement check and subsequent updates}
                        \State{$V_{t+1}(\beta'){=}\beta'{\cdot}\vec{\alpha}_{t+1}^{u};\ \vec{\alpha}_{t+1}(\beta'){:=}\vec{\alpha}_{t+1}^{u};\ \mathcal{K}_{t+1}(\beta'){:=}\mathcal{K}(\vec{\alpha}_{t+1}^{u}){\in}\mathcal{A}_{\Delta};\ \tilde{\mathcal{U}}{\longleftarrow}\tilde{\mathcal{U}}{-}\beta'$.}
                    \EndIf
                \EndFor
            \EndWhile
        \EndWhile
