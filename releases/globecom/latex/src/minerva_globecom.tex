...
$S_k(i)$ does not have constant amplitude, we may approximate $H_{k}(i)S_{k}(i)$ as complex Gaussian \add{with zero mean and variance $\sigma_H^2\mathbb E[|S_{k}(i)|^2]$}, without any modification to the subsequent analysis. \sst{$B_k(i)\in\{0,1\}$ is the binary spectrum occupancy variable, with $B_k(i)=1$ if the $k$th spectrum band is occupied by a PU at time $i$, and $B_k(i)=0$ otherwise.}}
\add{$B_k(i)\in\{0,1\}$ is the binary spectrum occupancy variable, with $B_k(i)=1$ if the $k$th spectrum band is occupied by a PU at time $i$, and $B_k(i)=0$ otherwise.}
 Therefore, the PU occupancy behavior in the entire wideband spectrum of interest \add{at time index $i$}, discretized into narrow-band frequency components, \sst{at time index $i$} can be modeled as the vector 
...
\nm{use "align" instead of "equation"}
\begin{align}\label{7}
&         \mathbb{P}(\vec{B}(i+1)|\vec{B}(i))\\&=
\nonumber
         \add{\mathbb{P}(B_{1}(i+1)|B_{1}(i))}
         \prod_{k=\add{2}}^{K}\ \mathbb{P}(B_{\add{k}}(i+1)|B_{\add{k}}(i),B_{k\add{-1}}(i+1)).
\end{align}
That is, \add{the spectrum occupancy at time $i+1$ in frequency band $k$,}
$B_{k}(i+1)$, depends on the  occupancy state of the adjacent spectrum band at the same time, $B_{k-1}(i+1)$, and that of the same spectrum band $k$ in the previous time index $i$, \add{$B_{k}(i)$}.

\add{\subsection{Spectrum sensing model}
In order to detect the available spectrum holes, the SU performs spectrum sensing. However,}
owing to physical design limitations at the SU's spectrum sensor \nm{citation?}, not all sub-bands in the discretized spectrum can be sensed.
\add{Therefore, due to limited sensing capabilities, the SU can sense only $\kappa$ out of $K$ spectrum bands at any given time, with $1\leq \kappa\leq K$.}
\sst{Let $\kappa$ with $1\leq \kappa\leq K$ be the number of spectrum bands that can be sensed by the SU at any time, capturing the spectrum sensing constraint.} Let $\mathcal K_{i}\subseteq\{1,2,\dots,K\}$ with $|\mathcal K_i|\leq \kappa$ be the set of indices corresponding to the spectrum bands sensed by the SU \add{at time $i$},  which is part of our design. Then, we model the emission process of the HMM as
\begin{equation}\label{8}
    \vec{Y}(i)\ =\ [Y_k(i)]_{k\in\mathcal K_i},
\end{equation}
\add{where $Y_k(i)$ is given by \eqref{3}.}

The true states \add{$\vec{B}(i)$} encapsulate the actual occupancy behavior of the PU and the measurements at the SU are noisy observations of these true states which are modeled to be the observed states of a Hidden Markov Model (HMM).
...
\end{equation}
owing to the \add{independence of} channels, noise and transmitted symbols across frequency bands. 
\nm{it should not be too difficult to incorporate channel correlation across frequency bands}
Moreover,
...
The agent's limited observational capabilities coupled with its noisy observations \add{and limited sensing capabilities} result in an increased level of uncertainty at the agent's end about the occupancy state of the spectrum under consideration and the exact effect of executing an action on the radio environment. The transition model of the underlying MDP as described in the previous subsection \add{(transition model of $\vec{B}(i)$)}, is denoted by $A$ and is learnt by the agent by interacting with the radio environment. The emission model \add{is denoted as and} $B$\nm{change notation, you use $B$ already for occuancy vector} is given by (\ref{9}), with $f(Y_k(i)|B_k(i))$ given by \eqref{10}. We model the POMDP as a tuple \nm{dont change notation, keep using $B$ for your states}
 $(\add{\mathcal B},\mathcal{A},\mathcal{Y},A,B)$ where $\add{\mathcal{B}}\equiv\{0,1\}^K$ represents the state space of the underlying MDP with states \sst{$\vec{x}$}\add{$\vec{B}$} given by all possible realizations of the spectrum occupancy vector as given by (\ref{3}), $\mathcal{A}$ represents the action space of the agent, given by all $\left(\begin{array}{c}K\\\kappa\end{array}\right)$ possible combinations in which the $\kappa$ spectrum bands are chosen to be sensed out of $K$ at any given time index; and $\mathcal{Y}$ represents the observation space of the agent based on the \sst{Observation Model}\add{observation model}\nm{no need for capitals} outlined in the previous subsection. \sst{The run-time or interaction time of the agent is quantized into discrete time indices.} \nm{confusing,, yu have already introduced $i$}
 At the beginning of each time index $i$, the agent selects $\kappa$ spectrum bands out of $K$, thus defining the sensing set $\mathcal K_i$, performs spectrum sensing  on these spectrum bands, observes \sst{$\vec{y} \in \mathcal{Y}$}\add{$ \vec{Y}(i)\in \mathcal{Y}$}\nm{keep consistent with the notation}, and updates its belief 
 \add{of the current spectrum occupancy $ \vec{B}(i)$ as}
\sst{ $\forall \vec{x}'$} as
\begin{equation}\label{11}
    \begin{aligned}
        b_a^{\add{\vec{Y}(i)}}(\add{\vec{b}}')\ =\ \mathbb{P}(\add{\vec{B}(i)=\vec{b}}'|\add{\vec{Y}(i)},\add{\mathcal K_i,}a\nm{what is this?},\add{b}\nm{no need for vec})\ =\ 
        \nm{fix:}\frac{\mathbb{P}(\vec{y}|\vec{x}',a)}{\mathbb{P}(\vec{y}|a,\vec{b})}\sum_{\vec{x} \in \mathcal{X}}\mathbb{P}(\vec{x}'|\vec{x},a)b(\vec{x}),
    \end{aligned}
\end{equation}
