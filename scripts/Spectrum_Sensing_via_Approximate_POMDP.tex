...
We introduce the model of PU occupancy over time and across frequency by modeling each $X_k(i)$ as 
...
\end{equation}
The PUs join and leave the spectrum at random times. To capture this temporal correlation in the spectrum occupancy dynamics of the PUs, we model $\vec{B}(i)$ as a Markov process,
...
In order to detect the available spectrum holes, the SU performs spectrum sensing. However, owing to physical design limitations of its spectrum sensor \cite{5990482}, the SU can sense at most $\kappa$ out of $K$ spectrum bands at any given time, with $1{\leq}\kappa{\leq}K$. Let $\mathcal K_{i}{\subseteq}\{1,2,\dots,K\}$ with $|\mathcal K_i|{\leq}\kappa$ be the set of indices of spectrum bands sensed by the SU at time $i$, part of our design.
Then, we define the observation vector
\begin{equation}\label{8}
    \vec{Y}(i) = [Y_k(i)]_{k {\in} \mathcal K_i},
\end{equation}
where $Y_k(i)$ is given by \eqref{3}.
The true states $\vec{B}(i)$ encapsulate the actual occupancy behavior of the PUs, and the measurements $\vec{Y}(i)$ at the SU are noisy observations of these true states, which are modeled to be the observed states of an HMM. Given $\vec{B}(i)$ and $\mathcal K_i$, the probability density function of $\vec{Y}(i)$ is
...
and the optimal spectrum sensing action is $\mathcal K_t^{u^*}$, where $\beta\cdot\vec{\alpha}{=}\sum_{\vec{B}}\beta(\vec{B})\vec{\alpha}(\vec{B})$
 denotes inner product. The set of hyperplanes $\{\vec{\alpha}_{t}^{u}\}$ associated with the set $\tilde{\mathcal{B}}$ are to be improved over numerous iterations of PERSEUS. Specifically, given $\{\vec{\alpha}_{t}^{u}\}$ and the optimal spectrum sensing actions
...
\nm{Did you discretize Y?} \bk{Doesn't the definition of the channel indices in the frequency domain $k$ imply the discretization of $Y$?}
\vspace{-3.5mm}
\section{Numerical Evaluations}\label{IV}
\noindent{\bf Simulation Setup:}
We simulate a radio environment with $3$ PUs occupying a set of $18$ channels, each of bandwidth $\text{BW}{=}160 \text{kHz}$, according to a Markovian time-frequency correlation structure defined by the parameters $p{=}q{=}0.3$, and an SU intelligently trying to access the available white spaces. We set $\sigma_{V}^{2}{=}1$ for the noise, $\sigma_{H}^{2}{=}80$ for the Rayleigh fading variance, $P_{tx}{=}54 \text{dBm}$ for the transmit power of the PUs, and $\tilde{P}_{tx}{=}47 \text{dBm}$ for the transmit power of the SU. \nm{I think it would make more sense to define all this quantities for a specific setup, based on path-loss, noise spectral density, etc.} \bk{Do you mean a real-world scenario simulation with a particular network topology, flat fading, etc.?}. These values result in a SINR at the incumbent given by $\text{SINR}_{\text{PU}}{=}24 \text{dB}$ when there is no interference from the SU, and $\text{SINR}_{\text{PU}}{=}6 \text{dB}$ with SU interference. Assuming that the SU is always backlogged, each channel $k$ offers a throughput of $\text{BW}\log_{2}(1{+}\text{SINR}_{\text{SU}, k}){\approx}1$Mbps\nm{this is true only if the SINR is fixed.. but the SINR varies due to fading} for the SU, when channel $k$ is truly idle, where $\text{SINR}_{\text{SU}, k}{=}19 \text{dB}$ is the SINR at the SU's receiver for channel $k$; and PU $j$ on channel $k$ achieves a throughput of $\text{BW}\log_{2}(1{+}\text{SINR}_{\text{PU}_{j}, k}){\approx}1.2$Mbps\nm{only true if SINR is fixed..in practice, you choose a certain rate and if this rate is above capacity you get outage, otherwise u succeed.. compute expected throughput instead}, when there is no interference from the SU on channel $k$. We further assume that the SU accesses all channels that are deemed idle by the POMDP agent in any given time-step $i$, thereby giving us an SU throughput\nm{note that throughput is rate multiplied by success probability..you are doing something different here..} metric described as $C_{\text{SU}}{=}\sum_{k=1}^{K}(1{-}B_{k}(i))(1{-}\phi_{k}(\hat{\beta}_{i}))$Mbps\nm{this does not look Mbps...}. Additionally, we evaluate PU interference by defining an indicator variable $\mathcal{I}$ which is assigned a value of $1$ when the SINR at the receiver of PU $j$, for a specific channel $k$, at a given time-step $i$, drops below $15$dB, thereby giving us a total interference metric with respect to $J$ PUs and $K$ channels, which is described as $$I{=}\mathbb{E}_{i}\left[\frac{\sum_{j}\sum_{k}B_{j,k}(i)\mathcal{I}(\text{SINR}_{\text{PU}_{j}, k}(i){<}15\text{dB})}{\sum_{k}B_{k}(i)}\right].$$\nm{is the SINR before or after fading? I.e., is it the expected SINR with respect to fading and conditional on SU being idle or not, or is it the actual SINR including fading?}
