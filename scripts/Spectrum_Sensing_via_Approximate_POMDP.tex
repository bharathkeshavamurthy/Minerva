...
\end{align}
Overall, the correlation model is expressed by two Markov chains: one across time, parameterized by $q{=}\mathbb{P}(B_{k}(i{+}1){=}1|B_{k}(i){=}0)$, and the other across frequencies, parameterized by $p{=}\mathbb{P}(B_{k}(i{+}1){=}1|B_{k{-}1}(i{+}1){=}0)$. We estimate these parameters $p$ and $q$, using the parameter estimation algorithm described in Sec. \ref{III} in order to obtain the transition model underlying the MDP, given by \eqref{7}.

\noindent \bk{Address the dual chain structure here: Spatial matrix using $p$ and $\Pi$, and temporal matrix using $q$ and $\Pi$.}
...
\end{equation}
where $\vec{\theta}{=}[p\ q]^{T}$, $\mathbf{Y}=[\vec{Y}(i)]_{i{=}1}^{\tau}$, $\mathbf{B}=[\vec{B}(i)]_{i{=}1}^{\tau}$,
...
$$
This operation continues until the set $\tilde{\mathcal{U}}$ is empty. The PERSEUS iterations continue until the termination condition is met, i.e., $V_{t{+}1}(\beta){-}V_{t}(\beta){<}\eta,\ \forall \beta{\in} \tilde{\mathcal{B}},\ \eta{>}0$. The belief update procedure outlined in \eqref{11} is an essential aspect of POMDPs, which can turn into a performance bottleneck for large state spaces due to the inherent iteration over all possible states. In order to circumvent this problem, we fragment the spectrum into smaller, independent sets of correlated channels and then run the PERSEUS algorithm on these fragments by leveraging multi-processing and multi-threading tools available at our disposal in software frameworks. Furthermore, we avoid iterating over all possible states and allow only those state transitions we deem to be the most probable -- for example, only those state transitions that involve a Hamming distance of up to $3$ between the previous and current state vectors in a radio environment with $18$ frequency channels.
\vspace{-4mm}
\section{Numerical Evaluations}\label{IV}
\noindent{\bf Simulation Setup:}
We simulate a radio environment with $J{=}3$ PUs occupying a set of $K{=}18$ channels, each of bandwidth $\text{BW}_{k}{=}160 \text{kHz}$, according to a Markovian time-frequency correlation structure defined by the parameters $p{=}q{=}0.3$, and a SU intelligently trying to access the available white spaces. We model the expected SINRs with respect to channel $k$ at time-step $i$, subject to fading, at the SU and the incumbents as: $\text{SINR}_{\text{PU}_{j},k}(i){=}17\text{dB}$ at PU $j$ when there is no interference from the SU, and $\text{SINR}_{\text{PU}_{j},k}(i){=}6\text{dB}$ with SU interference; and $\text{SINR}_{\text{SU},k}(i){=}11\text{dB}$ at the SU when the channel is not occupied by an incumbent, and $\text{SINR}_{\text{SU},k}(i){=}{-}6\text{dB}$ with PU interference. We assume that the SU is always backlogged and that it accesses all the channels deemed idle by the optimal POMDP policy. The SU throughput in a given time-step $i$ is described as $$C^{\text{SU}}(i) = \sum_{k=1}^{K} \tilde{R} \cdot \mathbb{P}\left(\text{SINR}_{\text{SU},k}(i) \geq 2^{\tilde{R}/\text{BW}_{k}} - 1\right),$$ where $\tilde{R}{=}0.6$Mbps is the transmission rate of the SU. Similarly, we evaluate the normalized throughput attained by the PUs in a given time-step $i$ as $$C^{\text{PUs}}(i) = \frac{\sum_{j}\sum_{k}R \cdot \mathbb{P}\left(\text{SINR}_{\text{PU}_{j},k}(i) \geq 2^{R/\text{BW}_{k}} - 1\right)}{\sum_{k}B_{k}(i)},$$ where $R{=}0.9$Mbps is the transmission rate of the PUs. \bk{I don't understand the comment on the probability being conditional on something.} Additionally, we impose a sensing restriction on the SU: at most $\kappa{=}6$ out of $K{=}18$ channels can be sensed by the SU in any given time-step $i$. Finally, while solving for the optimal policy in the PERSEUS algorithm, we employ a discount factor of $\gamma{=}0.9$ and a termination threshold of $\eta{=}10^{-5}$.

\noindent{\bf Evaluations:}
The plot depicted in Fig. \ref{fig:4} shows the mean square error (MSE) convergence of the parameter estimation algorithm to determine the parametric time-frequency correlation structure, i.e., $p$ and $q$. Starting with an initial estimate of $10^{-8}$, the EM algorithm iteratively reduces the MSE given by $\mathbb{E}[(\theta_{i}{-}\hat{\theta}_{i}^{(t)})^{2}],\ \theta_{i}{\in}[p,q]^{\intercal}$, as it goes through the E and M steps, and converges to the true transition model with an error of $\epsilon{\leq}10^{-8}$ over approximately $24000$ iterations. \bk{Talking about the run-time here means that we should talk about the platform, threading, software architecture, etc. Wouldn't a better metric be the Big-O notation?} Since the EM algorithm is susceptible to premature convergence to local optima and saddle points, we mitigate this by averaging the procedure over several cycles.
\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{PerseusRegretConvergence_MeanSquareErrorConvergence.png}
