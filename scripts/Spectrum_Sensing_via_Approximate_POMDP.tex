...
\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{MarkovChainsVisualization.png}
...
\nm{In your numerical results, you use a certain structure, not this generic one, you need to explain it here, i.e., define the parameters that define $\mathbb{P}(B_{k}(i+1)|B_{k}(i), B_{k-1}(i+1))$. "We further assume that...."}
\vspace{-3mm}
...
The true states $\vec{B}(i)$ encapsulate the actual occupancy behavior of the PU and the measurements at the SU are noisy observations of these true states which are modeled to be the observed states of an HMM.\sst{ 
Conditional on} \add{Given} $\vec{B}(i)$ and $\mathcal K_i$, the probability density function of $\vec{Y}(i)$ is\sst{ expressed as}
...
\end{equation}
\nm{I dont think this is accurate.. In your problem, you are actually doing the MLE of the parameters $p$ and $q$, and $\mathbf A$ is a function of those parameters...}
...
\end{equation}
This problem can be solved using the Expectation-Maximization (EM) algorithm,\nm{you need a reference for MLE of HMM} where the E-step constitutes
\begin{equation}
...
\end{equation}
\nm{again, the argmax should be over $p$ and $q$, not $\mathbf A$!}
\nm{In addition, we need mathematical equations for the E and M steps. "For the specific problem at hand, the E-step can be written as.... and the M-step can be written as ...."}
\subsection{The PERSEUS Algorithm}
\nm{Is this done after you have estimated the transition model? You need to explicitly say so}
As discussed in Sec. \ref{II} of this article, solving the Bellman equation \eqref{16} for POMDPs with large state and action spaces using exact value iteration and policy iteration techniques is computationally infeasible \cite{DBLP:journals/corr/abs-1109-2145}. Hence, we resort to approximate value iteration techniques to ensure that the system scales well to a large number of bands in the spectrum of interest. One such technique, the PERSEUS algorithm \nm{cite the reference here} is a randomized point-based approximate value iteration method that involves an initial phase of determining a set of so-called \emph{reachable beliefs} $\tilde{\mathcal{B}}$ by allowing the agent to randomly interact with the radio environment. The goal of the PERSEUS algorithm is to improve the value of all the belief points in this set $\tilde{\mathcal{B}}$ by updating the value of only a subset of these belief points, chosen iteratively at random. Using the notion that, for infinite-horizon POMDPs, $V^*$ in \eqref{16} can be approximated by a Piece-Wise Linear and Convex function (PWLC) \cite{DBLP:journals/corr/abs-1109-2145}, the PERSEUS algorithm operates on the core idea that the value function at time index $i$ can be parameterized by a set of hyperplanes $\{\vec{\alpha}_i^{u}\}$, $u {\in} \{0,1,2,\dots,|\tilde{\mathcal{B}}|\}$, each of which represents a region of the belief space for which it is the maximizing element. The belief points in $\tilde{\mathcal{B}}$ are to be improved over numerous iterative \emph{backup} stages. The optimal hyperplane\sst{ $\vec{\alpha}_{i+1}(\beta)$}\nm{stick with the notation!}
\add{$\vec{\alpha}_{i+1}^{u}$} at time index $i{+}1$ for \add{the $u$th} belief $\beta_{\add{u}} \in \tilde{\mathcal{B}}$ can be \add{iteratively} computed as
\nm{you need to cite reference again here}
\begin{equation}\label{20}
    \vec{\alpha}_{i+1}^{\add{u}}\mst{(\beta)} = \text{backup}(\beta_{\add{u}}) =\mst{ \argmax_{\vec{\alpha}_{i+1}^u, u {\in} \{0, 1, 2, \dots, |\tilde{\mathcal{B}}|\}} \beta \cdot \vec{\alpha}_{i+1}^u,}
 \add{\argmax_{\mathcal{K} \in \mathcal{A}} \beta_u \cdot \Xi_{\mathcal{K}}^{u},}
\end{equation}
\add{where $\beta\cdot\Xi=\sum_{\vec{B}}\beta(\vec{B})\Xi(\vec{B})$ denotes inner product,
$\Xi_{\mathcal{K}}^{u}$ is the hyperplane 
corresponding to a one-step look-ahead of the value iteration updates under action $\mathcal K$ and belief $\beta_u$, given by}
\begin{equation}
        \Xi_{\mathcal{K}}^{\add{u}} = \sum_{\vec{B} {\in} \mathcal{B}} \beta_{\add{u}}(\vec{B}) \mathbb{E}_{\vec{Y}|\vec{B}, \mathcal{K}} \Big[R(\vec{B}, \hat{\mathbb{B}}(\beta_{\add{u}}, \mathcal{K}, \vec{Y}))+\gamma 
        \add{\Xi_{\mathcal{K}, \vec{Y}}^{u}(\vec{B})\Big],}
\mst{        \argmax_{\Xi_{\mathcal{K}, \vec{Y}}^{u}, u {\in} \{0, 1, 2, \dots, |\tilde{\mathcal{B}}|\}} \beta \cdot \Xi_{\mathcal{K}, \vec{Y}}^{u}\Big],\ \text{where}}
\nonumber
\end{equation}
\nm{why was u starting from 0? Your set should have $|\tilde{\mathcal{B}}|$ elements }
\add{where $\Xi_{\mathcal{K}, \vec{Y}}^{u}$ is the hyperplane associated to the future value function, computed 
from the previous set of hyperplanes as
$$
\Xi_{\mathcal{K}, \vec{Y}}^{u}=\arg\max_{u' {\in} \{1, 2, \dots, |\tilde{\mathcal{B}}|\}} \mathbb{B}(\hat{\mathbb{B}}(\beta_{\add{u}}, \mathcal{K}, \vec{Y}))\cdot\alpha_{i}^{u'}.
$$
Once these hyperplanes have been computed,
 the new value function at a generic belief $\beta$ and the corresponding policy can be computed as}
\begin{equation}\label{40}
V_{i+1}(\beta) =\!\!\!\!\!\!\!\!\max_{\mst{\vec{\alpha}_{i+1}^u, }u {\in} \{1, 2, \dots, |\tilde{\mathcal{B}}|\}}\! \beta \cdot \vec{\alpha}_{i+1}^u,
\ \pi_{i+1}(\beta) = a(\vec{\alpha}_{i+1}^{*}),\!\!
\end{equation}
where $a(\vec{\alpha}_{i+1}^{*})$ is the
 action corresponding to \add{maximizing} hyperplane $\vec{\alpha}_{i+1}^{*}$.
\sst{Eq. eqref{20} can be written as}
\begin{equation}
\mst{    \vec{\alpha}_{i+1}^{*}(\beta) = \argmax_{\Xi_{\mathcal{K}}^{\beta}, \mathcal{K} \in \mathcal{A}} \beta \cdot \Xi_{\mathcal{K}}^{\beta},\ \text{where}}
\end{equation}
\begin{equation}
    \begin{aligned}
\mst{        \Xi_{\mathcal{K}}^{\beta} = \sum_{\vec{B} {\in} \mathcal{B}} \beta(\vec{B}) \mathbb{E}_{\vec{Y}|\vec{B}, \mathcal{K}} \Big[R(\vec{B}, \hat{\mathbb{B}}(\beta, \mathcal{K}, \vec{Y}))}\\ &\mst{+\gamma \argmax_{\Xi_{\mathcal{K}, \vec{Y}}^{u}, u {\in} \{0, 1, 2, \dots, |\tilde{\mathcal{B}}|\}} \beta \cdot \Xi_{\mathcal{K}, \vec{Y}}^{u}\Big],\ \text{where}}
    \end{aligned}
\end{equation}
\nm{this is not correct! The second term $\argmax_{\Xi_{\mathcal{K}, \vec{Y}}^{u}, u {\in} \{0, 1, 2, \dots, |\tilde{\mathcal{B}}|\}} \beta \cdot \Xi_{\mathcal{K}, \vec{Y}}^{u}$ should have a dependence on $\vec {B}$ (since you are doing a sum over that outside of the square parentheses ) which is not showing.. I corrected in my version above.. Please be more careful when you take equations from other papers, and always double check and question their correctness}
\begin{equation}
\mst{    \Xi_{\mathcal{K}, \vec{Y}}^{u}(\vec{B}) = \sum_{\vec{B}' \in \mathcal{B}} \mathbb{P}(\vec{Y}|\vec{B}', \mathcal{K})\mathbb{P}(\vec{B}',|\vec{B},\mathcal{K})\vec{\alpha}_{i}^{u}(\vec{B}').}
\end{equation}
