...
\begin{figure}
    \centering
    \includegraphics[width=0.985\linewidth]{MarkovChainsVisualization}
    \vspace{-5mm}
...
\end{align}
Overall,\sst{ we structure} the correlation model\sst{ as} \add{is expressed by} two Markov chains: one across time, parameterized by $q{=}\mathbb{P}(B_{k}(i{+}1){=}1|B_{k}(i){=}0)$, and the other across frequencies, parameterized by $p{=}\mathbb{P}(B_{k}(i{+}1){=}1|B_{k{-}1}(i{+}1){=}0)$.\nm{I don't understand. The above Markov chain is described by 
$\mathbb{P}(B_{1}(i+1)|B_{1}(i))$ and 
$\mathbb{P}(B_{k}(i+1)|B_{k}(i), B_{k-1}(i+1))$. How do you express these in terms of p and q?}
 We estimate these parameters $p$ and $q$, using the parameter estimation algorithm described in Sec. \ref{III} in order to obtain the transition model underlying the MDP, given by \eqref{7}.\nm{again, I dont understand how \eqref{7} can be expressed in terms of p and q...}
...
Given the posterior belief $\hat{\beta}_i$, we estimate\sst{ the occupancy state of the discretized spectrum under consideration} \add{$\vec{B}(i)$} as\sst{ $\vec{B}(i)^{*}{=}\argmax_{\vec{B} {\in} \mathcal{B}} \hat{\beta}_{i}(\vec{B})$. Let $B_{k}(i)^{*}{=}\phi_{k}(\hat{\beta}_{i}) {\in} \{0, 1\}$ be the estimated state of channel $k$ at time $i$.}
\add{
$$
\phi(\hat{\beta}_{i})\triangleq \argmax_{\vec{B} {\in} \mathcal{B}} \hat{\beta}_{i}(\vec{B}),
$$
with the $k$th spectrum channel estimate given by $\phi_k(\hat{\beta}_{i})$.}
 If the \add{$k$th} channel is deemed to be idle as a result of this MAP estimation procedure, i.e., $\phi_{k}(\hat{\beta}_{i}){=}0$, the SU accesses the channel to deliver its network flows. Else, it leaves it untouched. Given the PU occupancy state $\vec{B}(i)$ and posterior belief $\hat\beta_i$, the reward metric of the POMDP is given by the number of \emph{truly idle} bands detected by the SU, accounting for the throughput maximization aspect of the agent's objective, and a penalty for \emph{missed detections} accounting for interference to the incumbents,
\begin{equation}
\nonumber
    R(\vec{B}(i), \hat{\beta}_i){=}\sum_{k=1}^{K} (1{-}B_k(i))(1{-}\phi_k(\hat{\beta}_{i})){-}\lambda B_k(i)(1 - \phi_k(\hat{\beta}_i)),
\end{equation}
where $\lambda{>}0$ represents a penalty\sst{ factor} \add{cost}. After performing data transmission, the SU computes the prior belief for the next time-step based on the dynamics of the Markov chain as
...
The goal is to determine an optimal spectrum sensing policy to maximize the infinite-horizon discounted reward, \add{with discount factor $0{<}\gamma{<}1$,}
\begin{equation}\label{14}
    \pi^{*}{=}\argmax_{\pi} V^{\pi}(\beta) \triangleq \mathbb{E}_{\pi} \Big[\sum_{i=1}^{\infty} \gamma^{i} R(\vec{B}(i), \hat{\beta}_i)|\beta_0 {=}\beta\Big],
\end{equation}
where\sst{ $0{<}\gamma{<}1$ is the discount factor,} $\beta_0$ is the initial belief, $\hat\beta_i$ is the posterior belief induced by policy $\mathcal K_i{=}\pi(\beta_i)$ and the observation $\vec{Y}(i)$ via $\hat\beta_i{=}\hat{\mathbb B}(\beta_i, \mathcal K_i, \vec{Y}(i))$, and we have defined the value function $V^{\pi}(\beta)$ under policy $\pi$ starting from belief $\beta$.
...
\mst{\vec{\theta}^{*} = \argmax_{\vec{\theta}} \mathbb{P}([\vec{Y}(i)]_{i=1}^{\tau}|\vec{\theta}),}
\add{    \vec{\theta}^{*} = \argmax_{\vec{\theta}} \log\Big(\sum_{\mathbf{B}} \mathbb{P}(\mathbf{B}, \mathbf{Y}| \vec{\theta})\Big),}
\end{equation}
where $\vec{\theta}{=}[p\ q]^{T}$, 
\add{$\mathbf{Y}=[\vec{Y}(i)]_{i{=}1}^{\tau}$, $\mathbf{B}=[\vec{B}(i)]_{i{=}1}^{\tau}$,}
and $\tau$ refers to the learning period of the parameter estimator: this can be equal to the entire duration of the POMDP agent's interaction with the radio environment implying simultaneous model learning or can be a predefined parameter learning period before triggering the POMDP agent.\sst{ Let $\mathbf{Y}=[\vec{Y}(i)]_{i{=}1}^{\tau}$ and $\mathbf{B}=[\vec{B}(i)]_{i{=}1}^{\tau}$. Re-framing eqref{17} as an optimization of the log-likelihood, we get $\vec{\theta}^{*} = \argmax_{\vec{\theta}} \log\Big(\sum_{\mathbf{B}} \mathbb{P}(\mathbf{B}, \mathbf{Y}| \vec{\theta})\Big).$}
...
\end{equation}
\sst{This can be obtained}\add{The term $Q(\vec{\theta}|\vec{\theta}^{(t)})$ can be computed} by employing the Forward-Backward algorithm \cite{778361} using the current estimate $\vec{\theta}^{(t)}$. The M-step constitutes
\begin{equation}
\vec{\theta}^{(t+1)}
 = \argmax_{\vec{\theta}} Q(\vec{\theta}|\vec{\theta}^{(t)}),
\end{equation}
which involves re-estimation of the maximum likelihood parameter\sst{s in} \add{vector} $\vec{\theta}$ using the statistics obtained from the Forward-Backward algorithm.
...
&+\gamma 
        \sum_{\vec{B}'}\mathbb{P}(\vec{B}(i+1){=} \vec{B}'|\vec{B}(i){=}\vec{B})
                \Xi_{\mathcal{K}, \vec{Y}}^{u}(\vec{B}')\mst{\mid\vec{B}}\Big]
\nonumber
\end{align*}
...
$$
This operation continues until the set $\tilde{\mathcal{U}}$ is empty. The PERSEUS iterations continue until the number of policy changes between consecutive iterations is below a specified threshold $\eta$.\nm{why are you using this stop criterion? Since this is a value iteration algorithm, only sufficiently small changes in the value function updates should determine convergence, NOT small changes in the policy. That is the case for policy iteration, where no change in policy implies convergence.} The belief update procedure outlined in \eqref{11} is an essential aspect of POMDPs, which can turn into a performance bottleneck for large state spaces due to the inherent iteration over all possible states. In order to circumvent this problem, we fragment the spectrum into smaller, independent sets of correlated channels and then run the PERSEUS algorithm on these fragments by leveraging multi-processing and multi-threading tools available at our disposal in software frameworks. Furthermore, we avoid iterating over all possible states and allow only those state transitions we deem to be the most probable -- for example, only those state transitions that involve a Hamming distance of up to $3$ between the previous and current state vectors in a radio environment with $18$ frequency channels.
\vspace{-4mm}
\section{Numerical Evaluations}\label{IV}
\noindent{\bf Simulation Setup:}
We simulate a radio environment with $J{=}3$ PUs occupying a set of $K{=}18$ channels, each of bandwidth $\text{BW}_{k}{=}160 \text{kHz}$, according to a Markovian time-frequency correlation structure defined by the parameters $p{=}q{=}0.3$, and a SU intelligently trying to access the available white spaces. We model the SINR\nm{are these the expected SINRs (with respect to fading), or are you assuming that SINR is deterministic given SU/PU access decision?} at incumbent $j$ with respect to channel $k$ at time $i$ to be $\text{SINR}_{\text{PU}_{j},k}(i){=}40 \text{dB}$\nm{40-30dB seems quite big in practical scenarios..} when there is no interference from the SU, and $\text{SINR}_{\text{PU}_{j},k}'(i){=}10 \text{dB}$ with SU interference. Similarly, we model the SINR at the SU with respect to channel $k$ at time $i$ to be $\text{SINR}_{\text{SU},k}(i){=}30 \text{dB}$ when the channel is truly idle, and $\text{SINR}_{\text{SU},k}'(i){=}{-}10 \text{dB}$ when the channel is occupied by an incumbent. We assume that the SU is always backlogged and that it accesses all the channels deemed idle by the optimal POMDP policy. The average\nm{average with respect to what? Conditional to what?} SU throughput in a given time-step $i$ is described as $$C^{\text{SU}}(i) = \sum_{k=1}^{K} \tilde{R} \cdot \mathbb{P}\left(\text{SINR}_{\text{SU},k}(i) \geq 2^{\tilde{R}/\text{BW}_{k}} - 1\right),$$ where $\tilde{R}{=}1.5$Mbps is the transmission rate of the SU. Similarly, we evaluate the normalized throughput attained by the PUs in a given time-step $i$ as $$C^{\text{PUs}}(i) = \frac{\sum_{j}\sum_{k}R \cdot \mathbb{P}\left(\text{SINR}_{\text{PU}_{j},k}(i) \geq 2^{R/\text{BW}_{k}} - 1\right)}{\sum_{k}B_{k}(i)},$$
\nm{probability with respect to what? Conditional to what?}
 where $R{=}2.1$Mbps is the transmission rate of the PUs. Additionally, We impose a sensing restriction on the SU: at most $\kappa{=}6$ out of $K{=}18$ channels can be sensed by the SU in any given time-step $i$. Finally, while solving for the optimal policy in the PERSEUS algorithm, we employ a discount factor of $\gamma{=}0.9$ and a termination threshold of $\eta{=}0$.

\noindent{\bf Evaluations:}
The plot depicted in Fig. \ref{fig:4} shows the mean square error (MSE) convergence of the parameter estimation algorithm to determine the parametric time-frequency correlation structure, i.e., $p$ and $q$. Starting with an initial estimate of $10^{-8}$, the EM algorithm iteratively reduces the MSE given by $\mathbb{E}[(\theta_{i}{-}\hat{\theta}_{i}^{(t)})^{2}],\ \theta_{i}{\in}[p,q]^{\intercal}$, as it goes through the E and M steps, and converges to the true transition model with an error of $\epsilon{\leq}10^{-8}$ over approximately $24000$ iterations\nm{how do you translate these iterations in practice? I.e., how much is that in seconds? If one iteration is 1s, this is going to take forever.. but if 1 iteration is 1ms, then it is more practical.}. Since the EM algorithm is susceptible to premature convergence to local optima and saddle points, we mitigate this by averaging the procedure over several cycles.
\begin{figure}
    \centering
    \includegraphics[width=0.985\linewidth]{PerseusRegretConvergence_MeanSquareErrorConvergence.png}
...
\begin{figure}
    ...
    \label{fig:8}
\end{figure}
In this letter, we formulate the optimal spectrum sensing and access problem in
\add{cognitive radios via POMDPs.}\sst{
 an AWGN observation model with multiple licensed users and a cognitive radio restricted in terms of its sensing capabilities, as a POMDP.} In a radio environment wherein the occupancy behavior of the incumbents is correlated across time and frequencies, we present a consolidated\nm{what do you mean by "consolidated"?} framework that employs the EM algorithm to estimate the transition model of this occupancy behavior, and leverage a fragmented PERSEUS algorithm with belief update heuristics to simultaneously solve for the optimal spectrum sensing and access policy. Through system simulations, we conclude that our framework, in addition to its superior performance compared to algorithms in the state-of-the-art, facilitates regulation of the trade-off between SU throughput and PU interference.
