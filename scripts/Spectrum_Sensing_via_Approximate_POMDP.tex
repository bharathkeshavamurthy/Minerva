...
\begin{figure}
    ...
    \caption{\sst{The norm}\add{Mean} square\add{d} error convergence of the parameter estimation algorithm\sst{ while determining} \add{to determine} the correlation model $\vec{\theta}$, and\sst{ the} Regret convergence of the fragmented PERSEUS algorithm with belief simplification.\nm{what do you mean by belief simlplification? You never talked about it...}
    \nm{xaxis: you use iteration $t$; yaxis: you use time index $i$. Please be consistent. I think you shoujlkd use $i$ as you do in the rest of the paper}
    \add{Here, $\mathbb E$ denotes a sample average with respect to 60000 independent realizations.
    \nm{left yaxis: use $\Vert
\vec{\theta}{-}\vec{\theta}^{(t)}
    \Vert_2^2$ instead of $\sum_{\theta{\in}\vec{\theta}}(\theta{-}\hat{\theta}^{(t)})^{2}]$}}
    }
    \vspace{-4mm}
    \label{fig:4}
\end{figure}
Fig. \ref{fig:4} also illustrates the \emph{Regret}\nm{the word "regret" is misleading. In the RL and Multi armed bandits literature, regret is the sum of the loss with respect to oracle over the entire observation window. Instead, you define it for a specific time $i$. Call it "Loss" instead or something like that..}
 convergence plot of the PERSEUS algorithm, on the same time scale and in the same simulation run, over several iterations $t$\nm{t or i?}, wherein the regret metric corresponds to the difference in utility obtained by our PERSEUS algorithm at a certain iteration $t$ in our simulation, denoted by $R_{P}^{(t)}(\vec{B}(i), \hat{\beta}_{i})$, and an \emph{Oracle} which knows the exact occupancy behavior of incumbents in the network, with utility $R_{O}(\vec{B}(i))$. This trace in Fig. \ref{fig:4}, similar to the \emph{Reward v/s Time} plot in \cite{DBLP:journals/corr/abs-1109-2145}, serves as a measure of convergence for our fragmented PERSEUS algorithm with simplified\nm{what do you mean by simplified?} belief updates and online model estimation.
