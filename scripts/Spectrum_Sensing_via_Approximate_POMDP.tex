...
We simulate a radio environment with $J{=}3$ PUs occupying a set of $K{=}18$ channels, each of bandwidth $\text{BW}_{k}{=}160 \text{kHz}$, according to a Markovian time-frequency correlation structure defined by the parameters $p{=}q{=}0.3$, and a SU intelligently trying to access the available white spaces. We model the expected SINRs with respect to channel $k$ at time-step $i$, subject to fading, at the SU and the incumbents as: $\text{SINR}_{\text{PU}_{j},k}(i){=}17\text{dB}$ at PU $j$ when there is no interference from the SU, and $\text{SINR}_{\text{PU}_{j},k}(i){=}6\text{dB}$ with SU interference; and $\text{SINR}_{\text{SU},k}(i){=}11\text{dB}$ at the SU when the channel is not occupied by an incumbent, and $\text{SINR}_{\text{SU},k}(i){=}{-}6\text{dB}$ with PU interference. We assume that the SU is always backlogged and that it accesses all the channels deemed idle by the optimal POMDP policy. The SU throughput in a given time-step $i$ is described as $$C^{\text{SU}}(i) = \sum_{k=1}^{K} \tilde{R} \cdot \mathcal{I}\left(\text{SINR}_{\text{SU},k}(i) \geq 2^{\tilde{R}/\text{BW}_{k}} - 1\right),$$ where $\tilde{R}{=}0.6$Mbps is the transmission rate of the SU. Similarly, we evaluate the normalized throughput attained by the PUs as $$C^{\text{PUs}} = \frac{\sum_{i}\sum_{j}\sum_{k}R \cdot \mathcal{I}\left(\text{SINR}_{\text{PU}_{j},k}(i) \geq 2^{R/\text{BW}_{k}} - 1\right)}{\sum_{i}\sum_{k}B_{k}(i)},$$ where $R{=}0.9$Mbps is the transmission rate of the PUs. Additionally, we impose a sensing restriction on the SU: at most $\kappa{=}6$ out of $K{=}18$ channels can be sensed by the SU in any given time-step $i$. Finally, while solving for the optimal policy in the PERSEUS algorithm, we employ a discount factor of $\gamma{=}0.9$ and a termination threshold of $\eta{=}10^{-5}$.

\noindent{\bf Evaluations:}
The plot depicted in Fig. \ref{fig:4} shows the norm square error convergence of the parameter estimation algorithm to determine the parametric time-frequency correlation structure, i.e., $\vec{\theta}$. Starting with initial estimates of $10^{-8}$ for all the components of $\vec{\theta}$, the EM algorithm iteratively reduces the norm square error given by $\mathbb{E}[\sum_{\theta{\in}\vec{\theta}}(\theta{-}\hat{\theta}^{(t)})^{2}]$, as it goes through the E and M steps, and converges to the true transition model with an error of $\epsilon{\leq}10^{-8}$ for all components over approximately $60,000$ iterations, which translates to an estimation period of 180s, assuming each complete observation sampling period is $3$ms. Since the EM algorithm is susceptible to premature convergence to local optima and saddle points, we mitigate this by averaging the procedure over several cycles.
\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{PerseusRegretConvergence_NormSquareErrorConvergence.png}
    \caption{The norm square error convergence of the parameter estimation algorithm while determining the correlation model $\vec{\theta}$, and the Regret convergence of the fragmented PERSEUS algorithm with belief simplification}
    \vspace{-4mm}
    \label{fig:4}
\end{figure}
