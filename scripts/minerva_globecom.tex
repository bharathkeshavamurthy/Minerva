...
\hat\beta_i\add{(\vec{B}')}&=\mathbb{P}(\vec{B}(i)=\vec{B}'|\vec{Y}(i),\mathcal K_i, \beta_i)\\&=
\nonumber
...
\end{align}
\add{We denote the function that maps the prior belief $\beta_i$ to the posterior belief $\hat\beta_i$ through the spectrum sensing action $\mathcal K_i$ and the observation signal $\vec{Y}(i)$ as $\hat\beta_i=\hat{\mathbb B}(\beta_i,\mathcal K_i,\vec{Y}(i))$.}
\sst{where $\mathbb{P}(\vec{Y}(i)|\mathcal{K}_i,\beta_{i-1})$ denotes the normalization constant and $\beta_{i-1}$ represents the belief of the agent prior to the observation $\vec{Y}(i)$, defined as a probability distribution over all possible states. }

Given the posterior belief $\hat{\beta}_i$, we employ a threshold based detection mechanism to determine whether a given channel is occupied ($\phi_k(\hat{\beta}_i) = 1$ \add{for channel $k$}) or idle ($\phi_k(\hat{\beta}_i) = 0$).\nm{can you add more details here on how $\phi$ is defined, or a reference to the section where it is discussed?} If a channel $k$ is deemed to be idle by this threshold based detection mechanism, the SU accesses it for the delivery of its network flows.\sst{ On the other hand, if a channel $k$ is deemed to be occupied, the SU} \add{Otherwise, it} leaves it untouched.\sst{ Furthermore, for utility evaluation, since relying on feedback from the radio environment will introduce unforeseen variables and additional dynamics into the problem, we use the state estimator detailed in Sec. ref{III} to determine the reference occupancy metrics of the incumbents (the reference estimated state vector) $\hat{\vec{B}}$ based on the observations $\vec{Y}(i)$ obtained from the POMDP agent's sensing action, i.e. $\kappa_i$.}\nm{??? I don't understand this last statement..} 
\add{Given the PU occupancy state $\vec{B}(i)$ and posterior belief $\hat\beta_i$, the reward metric of the POMDP is given by}
the number of \emph{truly idle} bands detected by the SU accounting for the throughput maximization aspect of the agent's end-goal and a penalty for \emph{missed detections} accounting for the incumbent non-interference constraint,\sst{ the reward to the agent is modeled as} \add{i.e.}
\begin{equation}\label{12}
    R(\mst{\hat{\vec{B}}(i)}\add{{\vec{B}}(i)}, \hat{\beta}_i) = \sum_{k=1}^{K}(1 - \mst{\hat{B}_k}\add{{B}_k}(i))(1-\phi_k(\hat{\beta}_{i})) - \lambda \mst{\hat{B}_k}\add{{B}_k}(i)(1 - \phi_k(\hat{\beta}_i)),
\end{equation}
\nm{why $\hat{\vec{B}}(i)$? It should be the true state ${\vec{B}}(i)$}
where $\lambda > 0$ represents the cost term penalizing the agent for missed detections, i.e. interference with the incumbent. After performing data transmission, the SU computes the prior belief for the next slot \add{based on the dynamics of the Markov chain} as
\begin{equation}\label{13}
    \beta_{i+1}\add{(\vec{B}')} = \mathbb{P}(\vec{B}(i+1) = \vec{B}'|\mathcal{K}_{i+1}, \hat{\beta}_{i}).
\end{equation}
\add{We denote the function that maps the posterior belief $\hat\beta_i$ to the prior belief $\hat\beta_{i+1}$ as $\beta_{i+1}={\mathbb B}(\hat\beta_i)$.}
\nm{why $\mathcal{K}_{i+1}$? It should be $\mathcal{K}_{i}$.. and actually it only depends on $\hat\beta_i$ but is independent of $\mathcal{K}_{i}$.}
The\sst{ optimization objective} \add{goal} of the problem at hand
\add{is to determine an optimal spectrum sensing policy to maximize the infinite-horizon discounted reward,}
\sst{ can be written as}
\begin{equation}\label{14}
    \pi^{*} = \mst{\argmin}\add{\argmax}_{\mst{\mathcal{K}_{i} \in \mathcal{A}}\add{\pi}}\ \add{V^{\pi}(\beta)\triangleq}\mathbb{E}_{\add{\pi}}\Big[\sum_{i = 0}^{\infty}\ \gamma^{i} \mst{R(\vec{B}(i), \mathcal{K}_{i})}
    \add{R({\vec{B}}(i), \hat{\beta}_i)}
    \ |\ \beta_0 = \beta\Big],
\end{equation}
\nm{why are you changing the reward definition? Before it has been defined as
$R({\vec{B}}(i), \hat{\beta}_i)$!}
where $0 < \gamma < 1$ is the discount factor, $\beta_0$ is the initial belief\add{, and $\hat\beta_i$ is the posterior belief induced by policy $\pi$, which}\sst{ The action policy $\pi$ of the agent} maps the beliefs $\beta_i$ to actions $\mathcal{K}_i$ at time $i$,\sst{ and is characterized by a} \add{and we have defined the} value function
 \add{$V^{\pi}(\beta)$ under policy $\pi$ starting from belief $\beta$.}
 \begin{equation}\label{15}
\mst{V^{\pi}(\beta)\ =\ \mathbb{E}_{\pi}\ \Big[\sum_{i = 0}^{\infty}\ \gamma^{i} R(\vec{B}(i), \pi(\beta_{i}))\ |\ \beta_0 = \beta\Big],}
\end{equation}
\nm{again, why are you changing the reward? Be consistent!}
...
\add{The optimal policy $\pi^*$ and the corresponding optimal reward $V^*(\beta)$ are the solutions of Bellman's optimality equation
$V^*=H[V^*]$, where we have defined the operator $V_{n+1}=H[V_n]$ as}
\begin{equation}\label{16}
    V_{n+1}(\beta)=\max_{\mathcal{K}\in\mathcal{A}}\mst{\Big[}\sum_{\vec{B}\in\mathcal{B}}\mst{R(\vec{B},\mathcal{K})}\beta(\vec{B})
    \add{\mathbb E_{\vec{Y}|\vec{B},\mathcal K}\add{\Big[}R(\vec{B},\hat{\mathbb B}(\beta,\mathcal K,\vec{Y}))}
    $$$$
    +\gamma\mst{ \sum_{\vec{Y}\in\mathcal{Y}}\mathbb{P}(\vec{Y}|\mathcal{K},\beta)}V_n(\mst{\hat{\beta}}\add{\mathbb B(\hat{\mathbb B}(\beta,\mathcal K,\vec{Y}))})\Big],\ \forall \beta.
\end{equation}
\nm{why are you changing the reward?}
\add{This problem can be solved using the value iteration algorithm, i.e., by solving \eqref{16} iteratively until convergence to a fixed point. However,}
