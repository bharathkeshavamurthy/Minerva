...
To ameliorate the complexity of the POMDP optimization, we employ the PERSEUS algorithm, an approximate value iteration method. We compare numerically the performance of the proposed framework with state-of-the-art MAP-based state estimators and correlation-coefficient based clustering algorithms, and demonstrate that the proposed system achieves the performance attained by MAP-based state estimators which possess prior knowledge about the correlation model; it outperforms the correlation-coefficient based state-of-the-art by an average of 78\%; and it surpasses a Neyman-Pearson Detector that assumes independence among channels, by an average of 24\%.
\end{abstract}
\begin{IEEEkeywords}
