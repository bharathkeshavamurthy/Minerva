...
#   a part of Minerva.
# Does assuming independence across channels and across episodes bode well for us in the long run in terms of episodic
#   utility analysis when there exists a Markovian Correlation Model across both frequency and time?
...
# Reason for visualization: In a system wherein there exists a Markovian Correlation Model in the PU Occupancy Behavior,
            ...
            for channel in range(0, self.number_of_channels):
                ...
                episodic_reward += ((1 - self.true_pu_occupancy_states[channel][episode]) *
                                   ...
                                   (self.penalty * (1 - estimated_state[channel]) *
                                    self.true_pu_occupancy_states[channel][episode])
            utilities.append(episodic_reward)
        return utilities
...
class NeymanPearsonDetectorEvaluation(object):
    ...
    def setup_markov_chain(_correlation_class, _pi, _p):
        print('[INFO] NeymanPearsonDetectorEvaluation setup_markov_chain: Setting up the Markov Chain...')
    ...
    # The initialization sequence
    def __init__(self):
        print('[INFO] NeymanPearsonDetectorEvaluation Initialization: Bringing things up...')
    ...
    def evaluate(self):
        ...
        figure_url = plotly.plotly.plot(visualization_figure,
                                        filename='Episodic_Utilities_of_Neyman_Pearson_Detector')
        # Print the URL in case you're on an environment where a GUI is not available
        print('[INFO] NeymanPearsonDetectorEvaluation evaluate: '
